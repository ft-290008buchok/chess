...
int main() {
	...
	while (true) {
		...
		bool check = 0;
		for (i = 0; i < 28; i++) {
			/*Если координата b есть среди элементов
			массива ходов, ферзь угрожает данному полю*/
		...
		if (check)
			...
			/*Поля, которые нужно найти это пересечения
			массивов ходов ладьи с обоих полей;
			аналогичный принцип используется в двух следующих блоках*/
		...
		cout << "Продолжить работу с программой? да -- 1 нет -- 0\n";
		cin >> check;
		if (!check) { break; }
	}
